...
export function buildDefenderSystemPrompt(input: DefenderPromptInput): string {
	if (input.mirrorLanguage === 'it') {
		const method = [
			...
			'3. Scegli UNA tecnica dal TOOLBOX adatta alla difesa (preferisci 9 counter letterale, 12 pickup-and-escalate, 5 falso disconoscimento, 6 eccezione ironica, 4 inversione antitetica, 3 faint praise).',
	...
	const method = [
		...
		'3. Pick ONE technique from the TOOLBOX fit for defense (prefer 9 literal counter, 12 pickup-and-escalate, 5 false reassurance, 6 conditional reversal, 4 antithetical substitution, 3 faint praise).',
